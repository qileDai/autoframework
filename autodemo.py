...
class AutoDemo:
    ...
    def readExcel(self,file_path):
        ...
        try:
            ...
            self.autodemo.interfaceTest(case_list, file_path)
    ...
    def interfaceTest(self,case_list, file_path):
        ...
        for case in case_list:
            ...
            if param == '':
                ...
                '''
                new_url = url + '?' + self.autodemo.urlParam(param)  # 请求报文
                '''
            ...
            if method.upper() == 'GET':
                ...
                '''
                res = self.autodemo.readRes(results, res_check)
            else:
                '''
            ...
            if 'pass' in res:
                ...
                '''
                self.autodemo.writeBug(case_id, interface_name, new_url, results, res_check)
        '''
        ...
        法在下面定义了，也加了注释
        '''
        self.autodemo.copy_excel(file_path, res_flags, request_urls, responses)
    ...
    def writeBug(bug_id, interface_name, request, response, res_check):
        ...
        coon = pymysql.connect(user='root', passwd='123456', db='bugfree', port=3306, host='127.0.0.1', charset='utf8')
        # 建立游标
        cursor = coon.cursor()
        # 执行sql
